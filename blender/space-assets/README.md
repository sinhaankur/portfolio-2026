...
|-------|------|--------|-------|
| X-wing fighter | `xwing.glb` | detailed | Player ship. Fuselage + tapered nose + sensor, transparent canopy + frame, R2 astromech dome, 4 open-X S-foil wings with engine nacelles + intake rings + bright recessed glow, 4 forward wingtip cannons, orange stripe accents, wing flutes + greebles, bevelled edges. Y-forward to match `SHIP_MODEL_BASIS_ROTATION`. Source length ≈ 6.26 units. |
